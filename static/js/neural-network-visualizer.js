...
function drawNeuralNetwork(parentElement, networkElementId, networkStyle, ...networkDef) {
    ...
    let networkSvg = document.createElementNS(SVG_NS, "svg");
    ...
    for (let i = 0; i < layers.length; i++) {
        ...
        for (let j = 0; j < layers[i].length; j++) {
            ...
            let neuronSvg = document.createElementNS(SVG_NS, "circle");
            ...
            if (i < layers.length -1) {
                for (let k = 0; k < layers[i + 1].length; k++) {
                    let synapseSvg = document.createElementNS(SVG_NS, "line");
